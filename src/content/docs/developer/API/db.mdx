...
This database is based on sqlite thus doesn't have a key-value style storage. 
(I have plan to provide key-value style API in the future for convenience.)
...
:::caution
Since the database API currently doesn't support key-value store. `dataType` isn't a unique key. 
Running `.add()` multiple times with the same data will create multiple entries.

If you need to make sure there is a single entry, you need to use read, update and delete APIs to manage the data.

Later I will provide a way to store key-value pairs directly.
:::
