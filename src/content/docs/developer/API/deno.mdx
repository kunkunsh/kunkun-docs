...
> However, using a CLI is not always ideal for those who prefer calling scripts directly like a typical library. The Kunkun API solves this by offering an abstraction in RPC style, allowing Deno scripts to be invoked as libraries rather than CLIs.

The shell API provides a function siganture `createDenoRpcAPI` that creates a Deno RPC API.

```ts
function createDenoRpcAPI<LocalAPI extends {}, RemoteAPI extends {}>(
  scriptPath: string,
  args: string[],
...
:::danger
TODO: Update sample code here, export the functions from `@kksh/api` instead
:::

Here is how the library will be written in Deno. Just provide an API object to `ProcessChannel`. 
...
const child = new ProcessChannel(stdio, mathLib)
```

In extension code, use `shell.createDenoRpcAPI` to start running a Deno script and get the API object.

```ts title="extension.ts"
import { shell } from "@kksh/api/ui/worker"; // iframe version works too
import { type MathLibAPI } from "./types.ts"
...
// 
api.add(1, 2).then(console.log); // 3
api.subtract(1, 2).then(console.log); // -1
```
