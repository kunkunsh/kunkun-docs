...
<FileTree>- src - deno-script.ts - index.ts - ...</FileTree>
...
`"shell:stdin-write"` and `"shell:kill"` are also required permissions as the deno command uses stdin to communicate with the extension, and extension needs to kill the process after it's done.
